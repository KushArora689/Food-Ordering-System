...
##lists for price
price_list = [['20','9.50','7.50', '17'], ['3','4','3', '15.50']]
foodprice = price_list[0]
drinksprice = price_list[1]
orderlist = []
...
##creating an innit function
    ...
    def open_menu(self):
        ...
        self.button_checkout = Button(self.my_frame2,image = self.img_checkout, cursor = CURSOR, bd  = 0, command = self.cart_order)
    ...
    def view_menu(self):
        ...
        for x in range (len(food)):
            self.food_order = Label(self.my_frame3, font=MENU_FONT, text = food[x]).grid(row = 3+x, column = 4)
            self.food_price = Label(self.my_frame3, font=MENU_FONT, text = foodprice[x]).grid(row = 3+x, column = 8)

        ## displaying the menu by using a for loop
        for x in range (len(drinks)):
            self.drinks_order = Label(self.my_frame3, font=MENU_FONT, text = drinks[x]).grid(row = 5+(len(food))+x, column = 4)
            self.drinks_price = Label(self.my_frame3, font=MENU_FONT, text = drinksprice[x]).grid(row = 5+(len(food))+x, column = 8)
        ...
        self.back_button.grid(row = 38, column = 5)
    ...
    def order_menu(self):
        ...
        self.checkout_btn = Button(self.my_frame4, image = self.check_btn, cursor = CURSOR, bd = BORDER, command = self.cart_order)
...
## defining a subclass, where everything is being inhrited from the previous class "Goode_brothers"

class final_cart(Goode_brothers):

    ## creating a function to display the items in cart
    ...
    def cart_order(self):
        ...
        ## creating a back button as an image
        self.img_back2 = PhotoImage(file = 'dip_images\\bbutton.png')
        self.back_button2 = Button(self.root3, image = self.img_back2, cursor = CURSOR, bd = BORDER, command = self.open_menu)
        self.back_button2.grid(column=0, row = 1)

        ## creating a label as an image
        self.img_cart = PhotoImage(file = 'dip_images\\your_cart.png')
        self.y_cart = Label(self.root3, image = self.img_cart).grid(column=3, row = 1)

        ## creating labels for titles, and creating spacing to make page look good
        Label(self.root3, text = "").grid(column = 1, row = 1)
        Label(self.root3, text = "").grid(column = 1, row = 2)
        Label(self.root3, text = "Food",font=COURIER).grid(column = 2, row = 3)
        Label(self.root3, text = "Drinks",font=COURIER).grid(column = 3, row = 3)
        Label(self.root3, text = "Price($)",font=COURIER).grid(column = 4, row = 3)
        Label(self.root3, text = "").grid(column = 1, row = 4)

        ## displaying what the user ordered using a for loop
        for x in range (len(orderlist)):
            Label(self.root3, text = (str(x+1)),font=COURIER).grid(column =0, row= x+5)
            Label(self.root3, text = (orderlist[x][0]), font=COURIER).grid(column=2, row = x+5)
            Label(self.root3, text = (orderlist[x][1]), font=COURIER).grid(column=3, row = x+5)
            Label(self.root3, text = (orderlist[x][2]), font=COURIER).grid(column=4, row = x+5)

            self.totalorderprice = 0
         ## calculating and displaying the price using a for loop
            for x in range (len(orderlist)):
                self.totalorderprice = self.totalorderprice + (orderlist[x][2])

            Label(self.root3, text = "",font=COURIER).grid(column = 3, row = x+8)
            Label(self.root3, text = self.totalorderprice,font=COURIER).grid(column = 3, row = x+10)
            Label(self.root3, text = "Total Price ($)",font=COURIER).grid(column = 3, row = x+9)
    ...
    def calculate_order(self):
        ...
        self.drinks_value = drinks.index(self.tkvar2.get())
        self.food_value = food.index(self.tkvar.get())

        ## calculating the total
        self.total_price = 0
        self.total_price = float(foodprice[self.food_value]) + float(drinksprice[self.drinks_value])
